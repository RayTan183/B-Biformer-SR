...
def regional_routing_attention_torch(
    ...
    # Swin Attention
    attn_swin = (query * scale) @ key.transpose(-2, -1)
    attn_swin = attn_swin + relative_position_bias.unsqueeze(0)
    if mask is not None:
        nW = mask.shape[0]
        attn_swin = attn_swin.view(bs, nW, nhead, kv_region_size, kv_region_size) + mask.unsqueeze(1).unsqueeze(0)
        attn_swin = attn_swin.view(-1, nhead, kv_region_size, kv_region_size)
        attn_swin = torch.softmax(attn_swin, dim=-1)
    else:
        attn_swin = torch.softmax(attn_swin, dim=-1)
    output_swin = (attn_swin @ value)
